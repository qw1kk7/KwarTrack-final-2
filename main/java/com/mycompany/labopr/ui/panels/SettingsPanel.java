...
/**
 * SettingsPanel - Rounded white panels, theme toggle, data management
 */
public class SettingsPanel extends JPanel implements UITheme.ThemeChangeListener {
    ...
    private void initComponents() {
        ...
        // Appearance section with rounded panel
        centerPanel.add(createAppearanceSection());
        centerPanel.add(Box.createVerticalStrut(30));

        // Data Management section with rounded panel
    ...
    private JPanel createAppearanceSection() {
        StandardPanelFactory spf = (StandardPanelFactory) panelFactory;
        JPanel section = spf.createRoundedPanel(Color.WHITE, 20); // Rounded corners
        ...
        themeModeLabel.setForeground(Color.BLACK); // always black
    ...
    private JPanel createDataManagementSection() {
        ...
        dataManagementLabel.setForeground(Color.BLACK);
    ...
    private void updateThemeButtonStates() {
        ...
        if (isDark) {
            ...
            lightModeBtn.setBackground(BUTTON_UNSELECTED);
            lightModeBtn.setForeground(TEXT_UNSELECTED);
        } else {
            lightModeBtn.setBackground(BUTTON_SELECTED);
            lightModeBtn.setForeground(TEXT_SELECTED);
            darkModeBtn.setBackground(BUTTON_UNSELECTED);
            darkModeBtn.setForeground(TEXT_UNSELECTED);
        }
    ...
    public void onThemeChanged() {
        ...
        for (Component comp : getComponents()) {
            if (comp instanceof JPanel) updatePanelBackground((JPanel) comp, bgColor);
            else if (comp instanceof JScrollPane scroll) {
                scroll.getViewport().setBackground(bgColor);
                if (scroll.getViewport().getView() instanceof JPanel viewPanel)
                    updatePanelBackground(viewPanel, bgColor);
            }
        }
        repaint();
    }
    ...
    private void updatePanelBackground(JPanel panel, Color bgColor) {
        if (!panel.getBackground().equals(Color.WHITE)) panel.setBackground(bgColor);
        for (Component comp : panel.getComponents()) {
            if (comp instanceof JPanel child) updatePanelBackground(child, bgColor);
        }
    }
